...
for v in all_vid:
  ...
  # Store the video uniformly tiled
  rows = 2
  cols = 2
  uniformly_tiled_video_name = 'uniformly-tiled-%s' % ind
  t.store_with_uniform_layout(v, uniformly_tiled_video_name, rows, cols)
...
for ind in processed_vid_inds:
  ...
  dur, num_cars = time_selection(uniformly_tiled_video_name, metadata_id, label)
  tot_dur.append(dur)
  tot_cars += num_cars
end = now()
