...
for v in all_vid:
  ...
  print('== Storing %s in a non-uniform layout ==' % bname, flush=True)
  ...
  # Now, we can use the metadata to tile around traffic.
  tile_around_traffic_name = 'traffic-tiled-smart-%s' % ind
  t.store_with_nonuniform_layout(v, tile_around_traffic_name, metadata_id, label)
...
for ind in processed_vid_inds:
  ...
  dur, num_cars = time_selection(tile_around_traffic_name, metadata_id, label)
  tot_dur.append(dur)
  tot_cars += num_cars
end = now()
