...
# Perform 10 selections on the video to "train" the incremental tiler
for i in range(10):
  for ind in processed_vid_inds:
    ...
    if i == 0:
      ...
      t.activate_regret_based_tiling(untiled_video_name, metadata_id)
    dur, num_cars = time_selection(untiled_video_name, metadata_id, label)
    t.retile_based_on_regret(untiled_video_name)

# Now retrieve objects of interest using TASM
tot_dur = []
tot_cars = 0
start = now()
for ind in processed_vid_inds:
